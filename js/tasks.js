...
dtaskData.forEach((item) => {
    ...
    if (!isCompleted) {
        ...
        coinsParagraph.innerHTML = `${item.coins} <i class="fa-solid fa-diamond"></i>`;
        ...
        coinsParagraph.classList.add('color');
        // coinsParagraph.style = "color: #00aaff;";
        coinsParagraph.innerHTML = `<i class="fa-solid fa-circle-check" style="color: #da1d1d;"></i>`;
        cardCoinsDiv.appendChild(coinsParagraph);
...
function handleTaskClick(id) {
    // Check if axionis exists in local storage
...
function addsee(id) {
    // Check if axionis exists in local storage and initialize if not
    let workLine = JSON.parse(localStorage.getItem('axionis')) || { seed: [] };
...
function send(id, inputed) {
    const Task = taskData.find(task => task.id === id);
    let workLine = JSON.parse(localStorage.getItem('axionis')) || { complete: [], coins: 0 }; // Ensure workLine is initialized
